...
While Sloppy Noto can interpret any delimiter-separated values format (.csv, .tsv, .tab, .txt, etc.) containing large quantities of numeric data, it was created primarily to produce audio files out of the raw datasets of various space probes by [The European Space Agency](https://www.esa.int/), [The National Aeronautics and Space Administration](https://nasa.gov), etc.
